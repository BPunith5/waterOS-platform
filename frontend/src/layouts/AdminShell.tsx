...
import { Cpu, BarChart2, Send, LogOut, UserCog } from 'lucide-react';
import { useAuth } from '@/context/AuthContext';
import { OceanBackground } from '@/components/water/OceanBackground';

const NAV = [
  { to: '/admin/devices', icon: Cpu, label: 'My Devices' },
  { to: '/admin/telemetry', icon: BarChart2, label: 'Telemetry' },
  { to: '/admin/push', icon: Send, label: 'Push Reading' },
];
    ...
    <div className="relative flex min-h-screen">
      ...
      >
        <div className="flex items-center gap-2.5 px-5 py-5 border-b" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background: 'rgba(34,211,238,0.15)', border: '1px solid rgba(34,211,238,0.3)' }}>
            <UserCog size={16} color="#22D3EE" />
          </div>
          <div>
            <p className="text-sm font-bold leading-none" style={{ color: '#F4FBFF', fontFamily: 'var(--font-heading)' }}>WaterOS</p>
            <p className="text-[10px] mt-0.5" style={{ color: '#22D3EE', fontFamily: 'var(--font-body)' }}>Admin Panel</p>
          </div>
        </div>
